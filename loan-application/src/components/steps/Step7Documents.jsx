  ...
  return (
    ...
    >
      ...
      {fileRejections.length > 0 && (
        <p className="text-red-400 text-xs mt-2">⚠ Invalid file or file too large</p>
      )}
    </div>
  );
}
    ...
    <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-2xl p-5 mt-6">
      <div className="flex items-center justify-between mb-1">
        <div>
          ...
          <p className="text-[#5a5a5a] text-xs mt-0.5">Draw your signature below</p>
        </div>
        {hasSignature && (
          <button
            type="button"
        ...
        <div className="flex-1 border border-dashed border-[#3a3a3a] rounded-xl overflow-hidden">
          <SignatureCanvas
            ...
            canvasProps={{
              width: 900,
              height: 220,
              className: "w-full bg-[#1a1a1a] cursor-crosshair",
            }}
          />
        </div>

        {/* Preview panel — shown only after signing */}
        {signaturePreview && (
          <div className="w-52 flex-shrink-0 border border-[#1DB954]/30 rounded-xl bg-[#121212] flex flex-col items-center justify-center gap-2 p-3">
            <p className="text-[#5a5a5a] text-xs mb-1">Preview</p>
            <img
            ...
            />
            <p className="text-[#1DB954] text-xs mt-1">✓ Signature captured</p>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6 p-3 bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl">
        <div className="flex-1 bg-[#2a2a2a] rounded-full h-1.5">
          <div
            ...
            style={{
              width: `${(Object.keys(files).length / docConfig.length) * 100}%`,
            }}
          />
        </div>
      ...
      {submitted && !allRequiredUploaded && (
        <p className="text-red-400 text-xs mt-2">⚠ Please upload PAN and Aadhaar</p>
      )}
      ...
      <SignaturePad
        onSave={(img) => {
          setSignature(img);
          // Store in sessionStorage as the primary fallback channel
          if (img) {
            sessionStorage.setItem("loan_signature", img);
            localStorage.setItem("loan_signature", img);
          } else {
            sessionStorage.removeItem("loan_signature");
            localStorage.removeItem("loan_signature");
          }
          // shouldDirty + shouldTouch ensures RHF marks the field as changed
          // so watch() picks it up even across step mounts
          setValue("signature", img, {
            shouldDirty: true,
            shouldTouch: true,
            shouldValidate: true,
          });
      ...
      {submitted && !signature && (
        <p className="text-red-400 text-xs mt-2">⚠ Signature required</p>
      )}

      {/* Consent */}
      <div className="mt-5 p-4 bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl">
        <p className="text-[#b3b3b3] text-xs leading-relaxed">
          By uploading documents and signing above, I confirm all documents are genuine.
        </p>
      </div>
